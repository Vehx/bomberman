class Bomb {
    ...
    bombShapeTop() {
        ...
        for (let i = 0; i < numRect; i++) {
            ...
            rect(
                (this.x - shapeSize / 2 + (i > shapeSize ? i * 0.1 :- i) / shapeSize),
                (this.y - shapeSize + 2),
...
//the bigger the bomb is the longer it takes for it to explode (fuseTime)
    ...
    Show(fuseTime){
        ...
        push()
        if (fuseTime < this.FuseLength / 2.2) {
            push()
            translate(0, fuseTime)
            this.theFuse()
            this.fuseBurn()
            pop()
            this.bombShapeTop()
    ...
    explotionArea(){
       ...
       //y-axis cordinates
        ...
        for (let x2 = xleftEnd; x2 <= xrightEnd; x2+=iterationNumX) {
            for (let y2 = xHeightTop; y2 <= xHeightBottom; y2+=iterationNumX) {
